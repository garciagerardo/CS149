...
public class RoundRobin {
	...
	/**
	 * Runs RoundRobin algorithm on a list of processes; outputs nfo about run
	 */
	public void run() {
		timeCounter = 0;
		...
		while (!finished) {
			...
			if (!starved) {							// If any process is starved, current time exceeded 100 quanta. Don't add any more processes from queue to ready
				for (Process p: queue) {
			...
			if ((!starved && queue.isEmpty() && ready.isEmpty() && current == null) || (starved && ready.isEmpty() && current == null)) {
				finished = true;
			}
